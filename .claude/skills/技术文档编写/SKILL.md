...
### 适用
- 新建/改写 README、设计文档、实现指南、接口说明、架构说明
- 对已有技术文档做结构化整理（标题、目录、示例、术语统一）

### 不适用
- 数据库迁移 SQL 与配置递增管理（使用 `文档编辑规范`）
- Git 提交信息规范（使用 `Git提交规范`）
- 纯代码实现任务（使用开发类技能书）

## 标准产出（最小集）
- 1 份目标文档（Markdown）
- 1 份目录索引（目录 `README.md` 或 `INDEX.md`，包含顺序与简介）
- 1 段“变更摘要”（本次新增/修改/删除）
- 1 组“验证方式”（如何确认文档可用、链接可达、命令可执行）

## 跨技能一致约束（SQL 相关）
- 当技术文档涉及数据库版本变更时，必须与 `文档编辑规范` 保持一致。
- 禁止在文档中要求“直接修改已发布历史 `V*.sql`”。
- 必须采用“新增下一版本脚本 + 新增 rollback + 更新 `README.md`/`CHANGELOG.md` 索引与记录”的方式描述实施步骤。
...
### 新建文档编号与索引（强制）
- 同一目录下同类文档达到 3 个及以上时，必须使用序号前缀：`01-xxx.md`、`02-xxx.md`。
- 推荐两位序号，不足补零；需要长期扩展可用三位序号：`001-xxx.md`。
- 每个文档目录必须有 `README.md` 或 `INDEX.md`，提供按序号排序的导航目录（标题 + 一句话说明）。
- 新增、删除、重命名文档时，必须同步更新目录索引与交叉链接。

## 验收清单（提交前）
- [ ] 标题层级正确（H1 唯一，H2/H3 结构清晰）
- [ ] 命令/路径与当前仓库一致（可直接执行或定位）
- [ ] 链接为真实可用链接或明确占位（`<替换为...>`）
- [ ] 示例代码与正文语义一致（无过时字段/接口）
- [ ] 文档包含“适用范围/限制”或“前置条件”
- [ ] 新增文档已按规则编号（如 `01-xxx.md`）
- [ ] 所在目录 `README.md`/`INDEX.md` 已更新且可导航
- [ ] 若包含 SQL 变更说明：未要求改历史 `V*.sql`，而是新增版本并同步 rollback/索引/变更记录
