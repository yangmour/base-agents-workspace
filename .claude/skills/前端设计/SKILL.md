...
description: 前端页面与组件实现技能。凡是创建或修改 node-base-module 中的 Vue 后台管理页面、组件、布局、路由页面、表单、表格、仪表盘、可视化页面、交互状态、响应式样式、前端类型定义或接口调用封装时使用。只处理前端实现与前端验证；涉及后端接口字段变化时，先读取后端契约并同步前端最小必要适配，不扩展为独立全栈技能。实现时保留既有 Vue 3、TypeScript、Element Plus、Vite、Pinia、Vue Router 约定。
---

# 前端页面与组件实现

本技能用于项目内前端页面、组件、类型和接口调用封装开发，重点是可用、清晰、符合项目契约，并在不破坏既有设计系统的前提下提升界面质量。涉及后端接口字段变化时，直接核对后端契约并同步前端最小必要适配。
